...
//----------------------------------------------EVENTI PER I CONTROLLI--------------------------------------------------
        ...
        private void btn_no_Click(object sender, EventArgs e)
        {
            int colonne = int.Parse(txt_consumatore.Text);
            int righe = int.Parse(txt_produttore.Text);
            if (Convert.ToInt32(dtv_tabella.Rows[righe].Cells[0].Value) < Convert.ToInt32(dtv_tabella.Rows[0].Cells[colonne].Value))
            {
                dtv_tabella.Columns.Remove(dtv_tabella.Columns[0]);
            }
